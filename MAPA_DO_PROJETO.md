...
**Atualização 2026-08-20 (2):** CSS compartilhado entre `devolucao.html`/`privacidade.html`/`termos.html` extraído pra `/css/legal.css` — ver seção 9, nova. O JS duplicado dessas três páginas continua como estava, de propósito (é refatoração, não realocação — decisão adiada pra outra rodada). A seção também registra um flash de cor conhecido nessas três páginas (sem a injeção server-side que as outras páginas têm), não corrigido nesta rodada.
...
**`privacidade.html`, `termos.html`, `devolucao.html`** — páginas legais estáticas. CSS compartilhado via `/css/legal.css` (ver seção 9) — cada uma só mantém inline o que é específico dela. Preenchidas no cliente via `fetch('/api/config')` (nome, CNPJ, cores — mostram aviso se CNPJ não estiver cadastrado). Servidas cruas do cache em memória, sem templating server-side de conteúdo (isso inclui a cor: ver seção 9 sobre o flash de cor que essa ausência de templating causa).
...
## 9. Páginas legais: CSS compartilhado (2026-08-20) — e um flash de cor conhecido, não corrigido

`devolucao.html`, `privacidade.html` e `termos.html` tinham cada uma sua própria cópia de ~90 linhas de CSS, coladas à mão — a mesma causa raiz que já tinha gerado o bug do contador de carrinho (mudar a aparência exigia lembrar de editar em três lugares). Extraído pra `/css/legal.css` (rota em `server.js`, entrada em `vercel.json`, mesmo padrão dos `js/*.js`).

**As três cópias eram idênticas na parte compartilhada** (confirmado por diff, byte a byte) — as únicas diferenças eram regras específicas de cada página, cada uma com um elemento correspondente de verdade no corpo do arquivo, nenhuma órfã: `.aviso-escopo` (só `devolucao.html`, que tem um aviso "devolução ≠ troca"), e `h3`/`table` (só `privacidade.html`, pra tabela de compartilhamento de dados com terceiros). Essas regras específicas continuam inline em cada HTML, num `<style>` pequeno — só o que é comum às três foi pro arquivo compartilhado.

**Cache do arquivo novo é `public, max-age=300`, não `no-store`** — diferente do padrão usado em `js/cart.js`/`js/colors.js`. Motivo: aqueles arquivos JS acompanham páginas que são re-lidas/templadas por requisição; este CSS acompanha páginas servidas cruas do cache em memória, que nunca mudam com base em quem pediu — copiar o `no-store` sem pensar teria sido replicar um padrão certo pra um caso diferente.

**O JS que aplica nome/CNPJ/cor em cada página (`aplicarDadosLegal`/`carregarDadosLegal`) continua duplicado, deliberadamente fora desta rodada.** Ele não é um bloco `{}` isolado como o CSS era — cada página tem lógica própria (WhatsApp só em `devolucao.html`; cidade e um aviso de CNPJ inline só em `termos.html`) misturada dentro da mesma função compartilhada. Extrair isso direito exige separar um núcleo comum de um ponto de extensão por página — refatoração, não realocação — e foi adiado de propósito pra ser decidido como tal, não incluído de carona só por morar do lado do CSS.

**Flash de cor conhecido nessas três páginas, não corrigido nesta rodada.** Diferente de `VN_IMPORTS.html`/`search.html`/`produto.html`, que recebem a cor do lojista injetada pelo servidor na mesma resposta HTTP (`construirOverrideCoresStyle`, ver seção 6 — é exatamente a técnica que já eliminou esse tipo de flash nas outras páginas), as três páginas legais são servidas cruas, sem templating server-side de nenhum tipo. O `:root` de `/css/legal.css` tem os valores de fábrica fixos (tema "Creme Clássico"); a cor real só chega depois, via JS, depois do `fetch('/api/config')` responder. Existe uma janela real entre o primeiro paint (cor de fábrica) e a cor configurada aparecer — o mesmo problema que motivou a injeção server-side nas outras páginas, só que essas três ficaram de fora porque nunca passam pelo `server.js` como template, só como arquivo cru. A solução já existe no projeto (a mesma função `construirOverrideCoresStyle`, ou uma injeção equivalente, aplicada também a estas três) — não foi aplicada aqui de propósito, porque essa mudança altera comportamento visível (quando a cor aparece), e o pedido desta rodada era só mover código de lugar, sem alterar aparência.
